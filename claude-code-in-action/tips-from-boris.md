...
![1](https://github.com/Vishesh8/vishesh-db/raw/main/training/claude-code-in-action/1.webp)
...
![2](https://github.com/Vishesh8/vishesh-db/raw/main/training/claude-code-in-action/2.webp)
...
![3](https://github.com/Vishesh8/vishesh-db/raw/main/training/claude-code-in-action/3.webp)
...
![4](https://github.com/Vishesh8/vishesh-db/raw/main/training/claude-code-in-action/4.webp)
...
![5](https://github.com/Vishesh8/vishesh-db/raw/main/training/claude-code-in-action/5.webp)

7) I use slash commands for every "inner loop" workflow that I end up doing many times a day. This saves me from repeated prompting, and makes it so Claude can use these workflows, too. Commands are checked into git and live in .claude/commands/. For example, Claude and I use a /commit-push-pr slash command dozens of times every day. The command uses inline bash to pre-compute git status and a few other pieces of info to make the command run quickly and avoid back-and-forth with the model

🔗 https://code.claude.com/docs/en/slash-commands#bash-command-execution
![6](https://github.com/Vishesh8/vishesh-db/raw/main/training/claude-code-in-action/6.webp)
...
![7](https://github.com/Vishesh8/vishesh-db/raw/main/training/claude-code-in-action/7.webp)


10) We use a PostToolUse hook to format Claude's code. Claude usually generates well-formatted code out of the box, and the hook handles the last 10% to avoid formatting errors in CI later.
![8](https://github.com/Vishesh8/vishesh-db/raw/main/training/claude-code-in-action/8.webp)

11) I don't use --dangerously-skip-permissions. Instead, I use /permissions to pre-allow common bash commands that I know are safe in my environment, to avoid unnecessary permission prompts. Most of these are checked into .claude/settings.json and shared with the team.

![9](https://github.com/Vishesh8/vishesh-db/raw/main/training/claude-code-in-action/9.webp)

12) Claude Code uses all my tools for me. It often searches and posts to Slack (via the MCP server), runs BigQuery queries to answer analytics questions (using bq CLI), grabs error logs from Sentry, etc. The Slack MCP configuration is checked into our .mcp.json and shared with the team.
![10](https://github.com/Vishesh8/vishesh-db/raw/main/training/claude-code-in-action/10.webp)

13) For very long-running tasks, I will either (a) prompt Claude to verify its work with a background agent when it's done, (b) use an agent Stop hook to do that more deterministically, or (c) use the ralph-wiggum plugin (originally dreamt up by @GeoffreyHuntley).
![11](https://github.com/Vishesh8/vishesh-db/raw/main/training/claude-code-in-action/11.webp)
